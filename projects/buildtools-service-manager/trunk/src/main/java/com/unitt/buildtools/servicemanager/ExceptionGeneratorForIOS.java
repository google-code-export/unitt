...
public class ExceptionGeneratorForIOS implements CodeGenerator<DtoConfig>
{
    ...
    protected StringBuffer createInterfaceContents( DtoConfig aConfig )
    {
        ...
        {
            if ( isExceptionImport( importType ) )
            {
                out.append( "#import \"" + types.getTypeForPlatform( importType, PLATFORM ) + "\"\n" );
            }
        }
        out.append( "\n" );
        out.append( "\n" );
        ...
        // start
        out.append( "@interface " + aConfig.getGeneratedClassName() + " : NSError\n" );
        ...
        {
            if ( isExceptionField( property ) )
            {
                out.append( "\t" + types.getTypeForPlatform( property.getGetterType(), PLATFORM ) + "* " + property.getName() + ";\n" );
            }
        }
        out.append( "}\n" );
        ...
        for ( PropertyInfo property : aConfig.getDtoProperties().values() )
        {
            if ( isExceptionField( property ) )
            {
                out.append( "@property (" + getPropertyFlags( property ) + ") " + types.getTypeForPlatform( property.getGetterType(), PLATFORM ) + "* " + property.getName() + ";\n" );
            }
        }
    ...
    protected StringBuffer createImplementationContents( DtoConfig aConfig )
    {
        ...
        for ( PropertyInfo property : aConfig.getDtoProperties().values() )
        {
            if ( isExceptionField( property ) )
            {
                out.append( "@synthesize " + property.getName() + ";\n" );
            }
        }
        out.append( "\n" );
        ...
        for ( PropertyInfo property : aConfig.getDtoProperties().values() )
        {
            if ( needsRelease( property ) && isExceptionField( property ) )
            {
                out.append( "\t[" + property.getName() + " release];\n" );
            }
        }
        out.append( "\n" );
    ...
    protected boolean isExceptionField( PropertyInfo aProperty )
    {
        if ( "cause".equals( aProperty.getName() ) || "localizedMessage".equals( aProperty.getName() ) || "message".equals( aProperty.getName() ) || "stackTrace".equals( aProperty.getName() ) )
        {
            return false;
        }
    ...
    protected boolean isExceptionImport( String aImport )
    {
        return types.doesNonPrimitiveNeedImportForPlatform( aImport, PLATFORM ) && !"java.lang.StackTraceElement".equals( aImport );
    }
}
